...
def test_safe_agent_call_quarantine():
    ...
    assert "failed_tasks" in result, "Safe wrapper must return a 'failed_tasks' key for LangGraph aggregation"
    assert len(result["failed_tasks"]) == 1
    
    failure = result["failed_tasks"][0]
    assert failure["status"] == "failed"
    assert failure["task_id"] == "task-123"
    assert "Agent Crash Guard" in failure["result"]
    assert "tool_failure" in failure["result"]
