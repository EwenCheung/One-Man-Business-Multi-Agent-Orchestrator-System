...
def test_retriever_returns_completed_task():
    ...
    task = {"task_id": "1", "description": "Fetch stock for laptops", "assignee": "retriever", "status": "pending", "result": ""}
    result = retriever_agent(task)
...
def test_policy_returns_completed_task():
    ...
    task = {"task_id": "2", "description": "Check discount policy", "assignee": "policy", "status": "pending", "result": ""}
...
def test_memory_read_mode():
    ...
    task = {"task_id": "3", "description": "Search past negotiations", "assignee": "memory", "status": "pending", "result": ""}
    result = memory_agent_node(task)
...
def test_memory_update_mode():
    ...
    state = {"raw_message": "Hello", "reply_text": "Hi there", "sender_name": "Test"}
...
def test_reply_returns_text():
    ...
    state = {"sender_role": "customer", "completed_tasks": [], "soul_context": "", "rules_context": ""}
