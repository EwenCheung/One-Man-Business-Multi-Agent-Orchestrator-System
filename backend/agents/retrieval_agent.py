...
def _build_tools_for_request(role: str, sender_id: str):
    ...
    if "semantic_search_partner_agreements" in allowed_names:
        @tool
        def semantic_search_partner_agreements(query: str) -> str:
            """Find partner agreements semantically similar to the query, scoped to the current partner."""
            session = SessionLocal()
            try:
                return json.dumps(rt.semantic_search_partner_agreements(session, query, int(sender_id)), default=str)
            finally:
                session.close()
